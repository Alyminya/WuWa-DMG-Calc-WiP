class Moveset:
    ...
    def calculate_damage(self, base_attack, crit_damage):
        ...
        results['Heavy_Attack'] = {
            ...
            "crit_damage": base_attack * (self.heavy_attack["multiplier"] / 100) * (crit_damage / 100)
        }

        # Dodge Counter
        results['Dodge_Counter'] = {
            "damage": base_attack * (self.dodge_counter["multiplier"] / 100),
            "crit_damage": base_attack * (self.dodge_counter["multiplier"] / 100) * (crit_damage / 100)
        }

        # HA Resonance
        results['HA_Resonance'] = {
            "damage": base_attack * (self.ha_resonance["multiplier"] / 100),
            "crit_damage": base_attack * (self.ha_resonance["multiplier"] / 100) * (crit_damage / 100)
        }

        # HA Aftertune
        results['HA_Aftertune'] = {
            "damage": base_attack * (self.ha_aftertune["multiplier"] / 100),
            "crit_damage": base_attack * (self.ha_aftertune["multiplier"] / 100) * (crit_damage / 100)
        }

        # MidAir Attack
        results['MidAir_Attack'] = {
            "damage": base_attack * (self.midair_attack["multiplier"] / 100),
            "crit_damage": base_attack * (self.midair_attack["multiplier"] / 100) * (crit_damage / 100)
        }

        # Resonance Skill
        results['Resonance_Skill'] = {
            "damage": base_attack * (self.resonance_skill["multiplier"] / 100),
            "crit_damage": base_attack * (self.resonance_skill["multiplier"] / 100) * (crit_damage / 100)
        }
